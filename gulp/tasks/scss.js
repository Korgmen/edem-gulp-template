	...
	through.obj((file, enc, callback) => {
		try {
			...
			warnings.forEach((warning) =>
				console.warn(
					`\x1b[33m[lightningcss] ${warning.message} (${warning.loc.filename}:${warning.loc.line})\x1b[0m`,
				),
			);
...
export const scss = () => {
	return app.gulp
		.src(entries(), { sourcemaps: app.isDev })
		...
		.pipe(
			app.plugins.rename((file) => {
				if (file.basename === 'main') file.basename = 'style';
			}),
		)
