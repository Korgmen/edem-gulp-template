...
Настройка `editor.wordSeparators` считает разделителями слов и дефис, и подчёркивание: двойной клик и переход по словам останавливаются на частях БЭМ-класса `block__element--modifier` — блок, элемент или модификатор выделяется и заменяется отдельно.
