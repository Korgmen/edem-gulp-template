...
#### Команды
- `pnpm dev:deploy` — обычный режим разработки + автовыгрузка: каждый изменённый файл из `deploy.config.js` *(включая пересобранные gulp'ом файлы в `build`)* сразу отправляется на сервер;
- `pnpm run deploy` — продакшн-сборка *(окружение `prod`)* и полная выгрузка всех файлов из `deploy.config.js`;
...
> - Соединение держится открытым *(с keepalive)*, при обрыве *(например, переподключение VPN)* задача сама переподключается и повторяет загрузку через 1, 2, 5, 10 и 20 секунд;
