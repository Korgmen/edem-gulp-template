...
> В этом файле (`temp-theme.scss`) вы можете найти реализацию многих компонентов шаблона, а HTML-структуру компонентов можно увидеть в файле `src/html/_example.html`. Оформление компонентов там дано как пример — переносите нужное в `src/scss/components/`, а сам файл удаляйте.
...
И затем инициализировать слайдер по этому имени в файле `sliders.js` — селектор там выглядит как `[data-slider="example"]`.  
...
Само модальное окно создается через тег `<dialog>`, которому необходимо присвоить атрибут `id` со значением идентификатора модального окна и атрибут `data-modal` *(примеры модальных окон можно найти в файле `modals.html`)*.  
...
Само оповещение создается через добавление к любому блоку атрибута `popover` и атрибута `id`, со значением идентификатора оповещения *(примеры оповещений можно найти в файле `modal.html`)*. Так же необходимо задать атрибут `data-notify`.
